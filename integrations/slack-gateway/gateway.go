...
func (g *slackGateway) routes() http.Handler {
	...
	g.registerRoute(mux, "/settings/channels/", g.handleChannelSettings)
	g.registerRoute(mux, "/api/slack/install/selection", g.handleInstallTargetSelectionAPI)
	g.registerRoute(mux, "/api/slack/install/result", g.handleInstallResultAPI)
	g.registerRoute(mux, "/api/slack/workspaces", g.handleWorkspaceManagementAPI)
	g.registerRoute(mux, "/api/slack/workspaces/target", g.handleWorkspaceTargetAPI)
	g.registerRoute(mux, "/api/slack/workspaces/test", g.handleWorkspaceTestAPI)
	g.registerRoute(mux, "/api/slack/workspaces/disconnect", g.handleWorkspaceDisconnectAPI)
	...
	g.registerRoute(mux, "/internal/channel-actions/slack/reactions", g.handleSlackReactionAction)
	return mux
}
