...
import {
  resolveUpgradeRequestPath,
  rewriteConnectFrameAsTrustedProxyControlUi,
...
test("upgrade request path strips query strings before matching upstream pathname", () => {
  assert.equal(resolveUpgradeRequestPath("/gateway?foo=bar", "/gateway"), "/gateway");
  assert.equal(resolveUpgradeRequestPath("", "/gateway"), "/gateway");
});
